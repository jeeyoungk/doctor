...
import { createEnvChecker, type VersionRequirement, type BinaryChecker } from './doctor.js';
...
async function main() {
  ...
  for (const [displayName, requirement] of Object.entries(advancedRequirements)) {
    ...
    try {
      ...
      const status = result.satisfies ? '✅' : '❌';
      console.log(`${status} ${displayName}: ${result.currentVersion} satisfies ${requirement.operator}${requirement.version}`);
    } catch (error) {
